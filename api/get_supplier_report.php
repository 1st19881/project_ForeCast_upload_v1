...
function makePeriod($index, $forecastType, $firstDayOfWeek, $weekNo, $fcYear) {
    return [
        'key' => 'P' . $index,
...
/**
 * HEAD query: fetch period columns
 */
function fetchPeriods($conn, $plant, $supplier, $partNo, $startDate, $endDate) {
    ...
    foreach ($tempRows as $row) {
        ...
        $index = count($periods);
        $periods[] = makePeriod($index, $row['FORECAST_TYPE'], $row['FIRST_DAY_OF_WEEK'], $row['WEEK_NO'], $row['FC_YEAR']);
    }
...
/**
 * DATA query: fetch part rows with consumption quantities per period
 */
