...
├── report_custom.php            # หน้า Supplier Forecast Report (BOM + Forecast)
├── manage_auth.php              # หน้าจัดการสิทธิ์ผู้ใช้ (Admin เท่านั้น)
├── login.php                    # หน้า Login
├── check_type.php               # Diagnostic: ตรวจ type ของ FORECAST_DATE
├── get_customers_ajax.php       # AJAX endpoint โหลดลูกค้าจาก SAP ตาม Plant (Upload screen)
...
│   ├── get_suppliers.php        # JSON endpoint ดึง Supplier จาก EORD/PO
│   ├── get_supplier_report.php  # ดึงข้อมูลรายงาน Supplier Forecast (Consumption)
│   ├── get_supplier_report_drilldown.php # ดึงข้อมูลรายละเอียดการคำนวณ Consumption
...
| ERP Source | Direct Query | `INTRA01` | `Z_BOM_COMPONENT`, `Z_EORD_PO` | ข้อมูล BOM และ Supplier (Sync รายวัน) |
...
### `report_custom.php` - Supplier Forecast Report

- รายงานวิเคราะห์การใช้ชิ้นส่วน (Consumption) ของแต่ละ Supplier
- คำนวณจาก Forecast ของ FG x usage ใน BOM
- กรองข้อมูลตาม Plant, Supplier, Part Number และ Start Date
- รองรับการ Export เป็น Excel (SheetJS)
- **Drill-down:** คลิกที่จำนวนในตารางเพื่อดูว่า Consumption นั้นมาจาก FG Part ไหนบ้าง
...
### `GET api/get_suppliers.php`

ดึงรายชื่อ Supplier สำหรับ Select2
...
```json
[
  { "CODE": "10000001", "NAME": "Supplier Name A" }
]
```

### `GET api/get_supplier_report_drilldown.php`

ดึงรายละเอียดการคำนวณ Consumption รายชิ้น

**Query Params:** `plant`, `supplier`, `part_no`, `first_day_of_week`, `forecast_type`

**Response:**
```json
{
  "status": "success",
  "summary": { "total_forecast_qty": 500, "total_consump_qty": 1000 },
  "data": [
    { "FG_PART_NO": "FG123", "FG_PART_NAME": "FG Name", "FORECAST_QTY": 500, "USAGE_QTY": 2, "CONSUMP_QTY": 1000 }
  ]
}
```
...
| SheetJS (XLSX) | 0.18.5 | Export Excel จาก Client-side |
